...
# Plots.savefig(plot_scores,"C:\\Users\\Propietario\\Desktop\\ib\\5-Maestría\\GenData-PCA-UMAP\\FNN\\NN(S)_Params\\Red3_Plots_final_tanh\\Metricas_K-Folds.pdf")
...
# Extraemos los datos de df_datasignals que tengan σ > 0.5
df_datasignals_optim = df_datasignals[df_datasignals[!, :σs] .> 0.5, :]

df_datasignals_optim
num_datos = Int(size(df_datasignals_optim, 1))


datasignals_valid = Float32.(Matrix(df_datasignals_optim[10:step_valid:num_datos,1:3])')
datasignals = Float32.(Matrix(df_datasignals_optim[setdiff(1:num_datos, 10:step_valid:num_datos),1:3])')

σ_valid = df_datasignals_optim[10:step_valid:num_datos,4]
lcm_valid = df_datasignals_optim[10:step_valid:num_datos,5]

σ_col = df_datasignals_optim[setdiff(1:num_datos, 10:step_valid:num_datos),4]
lcm_col = df_datasignals_optim[setdiff(1:num_datos, 10:step_valid:num_datos),5]
...
pl_loss = Plots.plot(losses[300:end], xlabel = "Épocas", ylabel = "Loss", label = "Datos de entrenamiento",tickfontsize=12, labelfontsize=14, legendfontsize=12, framestyle =:box, gridlinewidth=1, xminorticks=10, yminorticks=10, size = (600,400))
Plots.plot!(losses_valid[300:end], xlabel = "Épocas", ylabel = "Loss", label = "Datos de validación", size = (600,400))
Plots.savefig(pl_loss,"C:\\Users\\Propietario\\Desktop\\ib\\5-Maestría\\GenData-PCA-UMAP\\FNN\\NN(S)_Params\\Red3_Plots_final_tanh\\Loss.pdf")
