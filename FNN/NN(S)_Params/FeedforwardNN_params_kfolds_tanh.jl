...
pl_loss = Plots.plot(losses[700:end], xlabel = "Épocas", ylabel = "Loss", label = "Datos de entrenamiento",tickfontsize=12, labelfontsize=14, legendfontsize=12, framestyle =:box, gridlinewidth=1, xminorticks=10, yminorticks=10, size = (600,400))
Plots.plot!(losses_valid[700:end], xlabel = "Épocas", ylabel = "Loss", label = "Datos de validación", size = (600,400))

Plots.savefig(pl_loss,"C:\\Users\\Propietario\\Desktop\\ib\\5-Maestría\\GenData-PCA-UMAP\\FNN\\NN(S)_Params\\Red3_Plots_final_tanh_zonaoptim\\Loss.pdf")
...
params_error = PlotlyJS.scatter(
    ...
    mode = "markers",
    hoverinfo = "text",
    hovertext = RMAE_scores_valid,
    marker = attr(
        color = RMAE_scores_valid,  # Use the color_vector for color mapping
...
params_layout = Layout(
    title = "Error relativo en la predicion datos de validacion",
